...
describe('History Integration (in-memory)', () => {
  ...
  describe('logs audit events on write operations', () => {
  ...
  describe('skips logging when item does not exist', () => {
    it('should not log UPDATE for a non-existent item', async () => {
      await itemService.update(999, { name: 'X' });
      expect(historyService.logs).toHaveLength(0);
    });

    it('should not log DELETE for a non-existent item', async () => {
